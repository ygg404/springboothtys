...
//		String s_id = UuidUtil.get32UUID();
        //获取文件后缀名
//		String s_fileType = s_filePathName.substring(s_filePathName.lastIndexOf("."));
        //组合最新文件名
        String s_newFileName = getFileName(s_filePathName);
        //组合文件保存路径
        String s_filePath = File.separator + s_folderPathFormat + File.separator + s_newFileName;
        if (isFullPath) {
            //组合文件保存路径
            String s_dstPath = BASE_PATH + File.separator + s_folderPathFormat;
            //硬盘路径是否存在，如果不存在则创建
            File file_dstFile = new File(s_dstPath);
            if (!file_dstFile.exists()) {
                file_dstFile.mkdirs();
            }
            s_filePathResult = BASE_PATH + s_filePath;
        } else {
            s_filePathResult = s_filePath;
        }
